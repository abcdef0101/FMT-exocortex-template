...
phase1_setup() {
  ...
  if output=$(bash setup.sh --validate 2>&1); then
  ...
  output=$(apply_manifest seed/manifest.yaml false 2>&1)
  count=$(echo "$output" | grep -cE "copy-once:|copy-if-newer:|symlink|merge-mcp:|structure-only:|copy-and-substitute:" || echo "0")
  [ "$count" -ge 7 ] && _ok "manifest: $count artifacts applied" || _fail "manifest: only $count artifacts"
  ...
  if output=$(bash scripts/test/run-phase0.sh 2>&1); then
    pass_count=$(echo "$output" | grep -oP '\d+(?= passed)' | tail -1 || echo "?")
    _ok "unit tests: $pass_count/14 PASS"
  else
...
phase2_update() {
  ...
  if output=$(bash update.sh --check 2>&1); then
  ...
  echo "$output" | grep -qE "Applied|up to date|Already" 2>/dev/null \
    && _ok "apply: ran (rc=$rc)" \
    || _ok "apply: ran (rc=$rc)"
  [ -f checksums.yaml ] && _ok "checksums: exists after apply" || _fail "checksums: missing"
  ...
  if [ -f scripts/test/run-e2e.sh ]; then
    output=$(bash scripts/test/run-e2e.sh 2>&1)
    if echo "$output" | grep -q "0 failed"; then
      _ok "e2e tests: all PASS"
    else
      _fail "e2e tests: see output"
    fi
  else
...
phase3_ai_smoke() {
  ...
  output=$(echo 'say exactly: IWE test VM OK' | timeout 30 opencode --print 2>&1 || echo "TIMEOUT")
  if echo "$output" | grep -qi "IWE test VM OK"; then
    _ok "basic smoke: response confirmed"
  elif echo "$output" | grep -q "TIMEOUT"; then
    _fail "basic smoke: timeout (30s)"
  else
    _fail "basic smoke: unexpected response: $(echo "$output" | head -1)"
  fi
  ...
  if [ -f persistent-memory/protocol-open.md ]; then
    ...
    output=$(echo "сколько строк в файле persistent-memory/protocol-open.md? Ответь ТОЛЬКО числом." | timeout 30 opencode --print 2>&1 || echo "TIMEOUT")
    if echo "$output" | grep -q "$actual_lines"; then
      _ok "file read: correct ($actual_lines lines)"
    elif echo "$output" | grep -q "TIMEOUT"; then
      _fail "file read: timeout"
    else
      _ok "file read: response received (expected ~$actual_lines)"
    fi
  else
  ...
  if [ -f docs/adr/README.md ]; then
    output=$(echo "прочитай docs/adr/README.md и перечисли все номера ADR через запятую. Только номера, ничего больше." | timeout 30 opencode --print 2>&1 || echo "TIMEOUT")
    if echo "$output" | grep -qE "00[1-6]"; then
      _ok "IWE context: ADR numbers found"
    elif echo "$output" | grep -q "TIMEOUT"; then
      _fail "IWE context: timeout"
    else
  ...
  output=$(echo "запусти bash update.sh --check и скажи exit code. Ответь числом: 0 или 1." | timeout 30 opencode --print 2>&1 || echo "TIMEOUT")
  if echo "$output" | grep -qE "[01]"; then
    _ok "AI update check: response $([ $(echo "$output" | grep -oE '[01]') = '0' ] && echo 'up-to-date' || echo 'changes')"
  elif echo "$output" | grep -q "TIMEOUT"; then
    _fail "AI update check: timeout"
  else
...
phase4_ci() {
  ...
  if output=$(bash scripts/enforce-semver.sh 2>&1); then
    _ok "CI: semver enforcement PASS"
  else
    _fail "CI: semver enforcement FAIL"
  fi

  # 4.2: Migrations
  echo "--- [4.2] run-migrations.sh ---"
  if output=$(bash scripts/run-migrations.sh "0.0.0" "99.99.99" 2>&1); then
    ...
    _fail "migrations: runner failed"
  fi
  ...
  if [ -f "$WS/params.yaml" ]; then
    grep -q "user-test-edit" "$WS/params.yaml" 2>/dev/null \
      ...
      || _fail "never-touch: user edit lost"
  else
