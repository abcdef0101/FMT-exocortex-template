...
if $DRY_RUN; then
    ...
    mkdir -p "$ENV_DIR"
    cat > "$ENV_FILE" <<EOF
# IWE configuration — generated by setup.sh $(date +%Y-%m-%d)
...
if $DRY_RUN; then
    ...
    find "$TEMPLATE_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" -o -name "*.service" -o -name "*.timer" \) | while read file; do
        sed_inplace \
