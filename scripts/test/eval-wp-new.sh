...
Ты — оценщик качества wp-new. Проверь что новый РП (WP-5 CI gates) записан
во все 5 мест атомарно. Оцени по 8 критериям.
Для каждого: score 0.0-1.0, passed (score ≥ threshold), reasoning (1-2 предл, русский).

## Критерии
$(cat "$RUBRICS")

## Состояние ДО создания WP-5 (seed)
=== WP-REGISTRY (должен обновиться) ===
$(cat "$DS_DIR/docs/WP-REGISTRY.md" 2>/dev/null || echo "N/A")
=== MEMORY.md (должен обновиться) ===
$(cat "$WS_DIR/memory/MEMORY.md" 2>/dev/null || echo "N/A")
=== WeekPlan (должен обновиться) ===
$(cat "$DS_DIR/current/WeekPlan"*".md" 2>/dev/null | head -60 || echo "N/A")
=== DayPlan (должен обновиться если активен) ===
