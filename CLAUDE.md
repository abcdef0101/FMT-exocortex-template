## 0. Сразу после старта

1. Прочитай симлинку `./workspaces/CURRENT_WORKSPACE`.
2. Если симлинка существует → извлеки имя рабочего пространства и продолжай.
3. Если симлинка не существует:
   a. Вызови скилл `iwe-workspace --get-workspaces`.
   b. Если список пуст — спроси пользователя, хочет ли он создать первое рабочее пространство. Если да → запусти `./setup.sh`.
   c. Если список не пуст — выведи список пользователю и попроси выбрать одно из имён.
   d. Вызови скилл `iwe-workspace --get-workspace <выбранное имя>`.
   e. Если имя не найдено — сообщи об ошибке и попроси повторить.
Текущий проект: iwe-workspace --get-current 

# Инструкции для всех репозиториев

> Slim-ядро: триггеры + правила. Детали → @./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/protocol-*.md, .claude/rules/, .claude/skills/.
...
Детали типов, именование, измерения: → `@./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/repo-type-rules.md`
...
### Протокол Работы (`@./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/protocol-work.md`)
...
≤15 мин — не нужен. Внутри системы — `@./workspaces/CURRENT_WORKSPACE/<repo>/PROCESSES.md`. Новая система — сценарий + процессы + данные.

## 4. Memory (Слой 3)

| Ситуация | Читай |
|----------|-------|
| Файлы/репо | `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/navigation.md` |
| Pack-репо | `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/repo-type-rules.md` |
| Терминология | `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/hard-distinctions.md` |
| FPF/SOTA/Роли | `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/fpf-reference.md`, `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/sota-reference.md`, `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/roles.md` |
| Документ/чеклист | `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/checklists.md` |
...
Рабочая директория: `<директория этого CLAUDE.md>` (не из sub-директорий). `<директория этого CLAUDE.md>/workspaces/CURRENT_WORKSPACE/memory/` = директория auto-memory.
...
- Протоколы → `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/protocol-*.md`
- Различение (1-3 строки) → `.claude/rules/distinctions.md`
- Форматирование → `.claude/rules/formatting.md`
- Стабильные знания → `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/*.md`
...
- `<директория этого CLAUDE.md>` — рабочая директория
...
Прямое редактирование `.claude/skills/` или `./workspaces/CURRENT_WORKSPACE/memory/persistent-memory/protocol-*.md` = ошибка.
...
## 10. Пользовательское расширение CLAUDE.md (пользовательское расширение, §10)
@./workspaces/CURRENT_WORKSPACE/CLAUDE.md
