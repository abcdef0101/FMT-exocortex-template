...
#   daily-report.sh --workspace-dir /path/to/workspace --dry-run # показать отчёт, не записывать
...
compute_traffic_light() {
  ...
  GREEN)
    emoji="🟢"
    label="Среда готова к работе"
    ;;
  YELLOW)
    emoji="🟡"
    label="Среда работает с замечаниями"
    ;;
  RED)
    emoji="🔴"
    label="Критический сбой — требуется внимание"
    ;;
  esac
