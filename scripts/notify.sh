...
if [[ -n "${IWE_NOTIFY_ENV_FILE:-}" ]] && [[ -f "${IWE_NOTIFY_ENV_FILE}" ]]; then
  iwe_load_env_file "${IWE_NOTIFY_ENV_FILE}" || true
elif _repo_root="$(iwe_find_repo_root "${SCRIPT_DIR}" 2>/dev/null)"; then
