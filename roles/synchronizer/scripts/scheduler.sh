...
# Состояние: $WORKSPACE_DIR/state/ (маркеры запуска)
#
# Использование:
#   scheduler.sh --workspace-dir /path/to/workspace --roles-dir /path/to/roles --env-file /path/to/env dispatch
#   scheduler.sh --workspace-dir /path/to/workspace --roles-dir /path/to/roles --env-file /path/to/env status
...
if [ ! -d "$ROLES_DIR" ]; then
  echo "Ошибка: ROLES_DIR не существует: $ROLES_DIR" >&2
  exit 1
fi
...
dispatch() {
  ...
  if ! ran_today "synchronizer-code-scan"; then
    ...
    if timeout "$TASK_TIMEOUT_SHORT" "$SCRIPT_DIR/code-scan.sh" --workspace-dir "$WORKSPACE_DIR" --env-file "$ENV_FILE" >>"$LOG_FILE" 2>&1; then
  ...
  if ! ran_today "synchronizer-dt-collect"; then
    ...
    if timeout "$TASK_TIMEOUT_SHORT" "$SCRIPT_DIR/dt-collect.sh" --workspace-dir "$WORKSPACE_DIR" --env-file "$ENV_FILE" >>"$LOG_FILE" 2>&1; then
  ...
  if ! ran_today "synchronizer-daily-report"; then
    if ran_today "strategist-morning" || ((10#$HOUR >= 6)); then
      ...
      if timeout "$TASK_TIMEOUT_SHORT" "$SCRIPT_DIR/daily-report.sh" --workspace-dir "$WORKSPACE_DIR" >>"$LOG_FILE" 2>&1; then
  ...
  if ((10#$HOUR >= 7 && 10#$HOUR <= 23)); then
    ...
    if [ "$elapsed" -ge 10800 ]; then
      ...
      if timeout "$TASK_TIMEOUT_LONG" "$EXTRACTOR_SH" --root-dir "$ROOT_DIR" --workspace "$WORKSPACE_NAME" --agent-ai-path "$AI_CLI_PATH" inbox-check >>"$LOG_FILE" 2>&1; then
...
status)
  ...
  ;;
*)
  echo "Usage: scheduler.sh --workspace-dir <path> --roles-dir <path> --env-file <path> {dispatch|status}"
  echo ""
