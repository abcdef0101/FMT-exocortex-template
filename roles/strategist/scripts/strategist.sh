...
${prompt}"
  ...
  if git -C "$STRATEGY_DIR" diff --quiet origin/main..HEAD 2>/dev/null; then
    ...
    git -C "$STRATEGY_DIR" pull --rebase >>"$LOG_FILE" 2>&1 && log "Pulled (rebase)" || log "WARN: pull --rebase failed"
...
acquire_lock() {
  ...
  if ! mkdir "$lockdir" 2>/dev/null; then
    ...
    if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
      ...
      rm -rf "$lockdir"
      mkdir "$lockdir" || {
        log "ERROR: failed to acquire lock for $scenario"
        exit 1
      }
    fi
  fi
  echo $$ >"$lockdir/pid" || {
    rm -rf "$lockdir"
    log "ERROR: failed to write PID for $scenario"
    exit 1
  }
...
RHYTHM_CONFIG="$WORKSPACE_DIR/memory/day-rhythm-config.yaml"
...
"note-review")
  ...
  FLEETING="$STRATEGY_DIR/inbox/fleeting-notes.md"
  ...
  if ! git -C "$STRATEGY_DIR" diff --quiet -- inbox/fleeting-notes.md archive/notes/Notes-Archive.md 2>/dev/null; then
    git -C "$STRATEGY_DIR" add inbox/fleeting-notes.md archive/notes/Notes-Archive.md
    git -C "$STRATEGY_DIR" commit -m "chore: auto-cleanup processed notes from fleeting-notes.md" >>"$LOG_FILE" 2>&1 || true
    git -C "$STRATEGY_DIR" pull --rebase >>"$LOG_FILE" 2>&1 && log "Cleanup: pulled (rebase)" || log "WARN: cleanup pull --rebase failed"
    git -C "$STRATEGY_DIR" push >>"$LOG_FILE" 2>&1 && log "Cleanup: pushed" || log "WARN: cleanup push failed"
  else
