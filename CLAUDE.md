...
3. Если симлинка не существует:
   a. Запусти `./scripts/switch-workspace.sh --get-workspaces`.
   ...
   d. Запусти `./scripts/switch-workspace.sh --set-workspace=<выбранное имя>`.
   e. Если имя не найдено — сообщи об ошибке и попроси повторить.
Текущий проект: ./workspaces/CURRENT_WORKSPACE 
...
- Протоколы → `memory/persistent-memory/protocol-*.md`
- Различение (1-3 строки) → `.claude/rules/distinctions.md`
- Форматирование → `.claude/rules/formatting.md`
- Стабильные знания → `memory/persistent-memory/*.md`
...
Прямое редактирование `.claude/skills/` или `memory/persistent-memory/protocol-*.md` = ошибка.
