...
overlay_worktree_snapshot "$TMPDIR/upstream"
commit_snapshot_if_dirty "$TMPDIR/upstream"
...
git -C "$TMPDIR/upstream" add -A && git -C "$TMPDIR/upstream" -c user.name=Test -c user.email=test@example.com commit -m "test: mock upstream change" --quiet
...
if [ -f "$UPDATE_SH" ]; then
  ...
  [ "$rc" -eq 1 ] \
    && _pass "update.sh --check: exits 1 when changes available" \
    || _fail "update.sh --check: rc=$rc (expected 1)"
  echo "$output" | grep -q "Changes available" 2>/dev/null \
    && _pass "update.sh --check: detected changes" \
    || _fail "update.sh --check: missing changes marker"
else
...
if [ -f "$UPDATE_SH" ]; then
  ...
  NEW_LOCAL_SHA=$(git -C "$TMPDIR/local" rev-parse HEAD 2>/dev/null)
  [ "$rc" -eq 0 ] \
    && _pass "update.sh --apply: exits 0" \
    || _fail "update.sh --apply: rc=$rc"
  echo "$output" | grep -q "Update Applied" 2>/dev/null \
    && _pass "update.sh --apply: applied successfully" \
    || _fail "update.sh --apply: missing success marker"
  [ "$NEW_LOCAL_SHA" = "$UPSTREAM_SHA" ] \
    && _pass "update.sh --apply: local HEAD moved to upstream" \
    || _fail "update.sh --apply: local HEAD did not reach upstream"
fi
