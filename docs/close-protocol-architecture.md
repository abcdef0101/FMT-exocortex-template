...
| Mechanical day-close operations | `day-close.sh` | `scripts/day-close.sh` |
...
**За что отвечает:** Полный алгоритм конкретного протокола. Порядок шагов, переходы, бизнес-правила обновления WeekPlan/MEMORY.md/DayPlan, checklist протокола, условия (multiplier_enabled, reflection_enabled, lesson_rotation). Именно здесь знание: что считать итогом дня, когда обновлять carry-over, что входит в compact dashboard, что проверять перед commit.
...
**Файлы:** `scripts/day-close.sh`.

**За что отвечает:** Выполнить детерминированные операции, которые лучше делает shell, а не LLM: backup workspace memory/ (MEMORY.md, day-rhythm-config.yaml, etc.) → exocortex/, knowledge-MCP reindex, linear sync. Свести несколько рутинных операций в один вызов из skill.
...
**За что отвечает:** Кастомизация поведения без редактирования платформенного ядра. Before-расширения (утренние ритуалы), after-расширения (рефлексия), checks (дополнительные проверки), параметры (multiplier_enabled, lesson_rotation, reflection_enabled).
...
```
триггер «закрываю день» → hook → /day-close или /run-protocol close day → run-protocol загружает extensions → day-close/SKILL.md (full algorithm via TodoWrite) → шаг 4: scripts/day-close.sh (backup/reindex/linear) → checks + commit/push → compact dashboard → verifier (Haiku R23)
```
...
| 6. Helpers | Выполнять механические операции (backup, reindex) | Решать, закрыт ли протокол |
...
- `scripts/day-close.sh` отвечает за **как выполнить механические подшаги**
