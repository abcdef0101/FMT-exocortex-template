...
4. **Чеклист-верификация (R23):** Quick Close и Day Close — `bash scripts/verify-close.sh`. Проверяет формальное соответствие чеклисту (все ли пункты закрыты, есть ли коммит, обновлён ли MEMORY.md), но не оценивает качество результата. Исключения: сессия ≤15 мин или без изменений файлов.
...
| РП затрагивает PII (email, telegram_id, ЦД, tokens, user_events) | **Security Gate (B7.3):** ответить на §Б чеклист ArchGate ДО реализации. PII logging = blocker. |
...
| Ситуация | Читай |
|----------|-------|
| Файлы/репо | `./persistent-memory/navigation.md` |
| Pack-репо | `./persistent-memory/repo-type-rules.md` |
| Терминология | `./persistent-memory/hard-distinctions.md` |
| FPF/SOTA/Роли | `./persistent-memory/fpf-reference.md`, `./persistent-memory/sota-reference.md`, `./persistent-memory/roles.md` |
| Документ/чеклист | `./persistent-memory/checklists.md` |
...
> **3 слоя:** L1 (§1-§7) = платформа (`update.sh --check` / `--apply`). L2 (§8) = staging. L3 (§9) = авторское.

- Протоколы → `./persistent-memory/protocol-*.md`
