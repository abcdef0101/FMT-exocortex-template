...
**За что отвечает:** Подтвердить, что протокол завершен формально корректно. Проверить checklist: все ли шаги выполнены, есть ли commit, обновлен ли MEMORY.md. Исполняется `bash scripts/verify-close.sh` (R23, детерминированная проверка фактов).
