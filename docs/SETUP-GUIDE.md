...
# OpenCode со стандартным провайдером (DeepSeek, Anthropic, etc.)
export AI_CLI=opencode
export AI_CLI_MODEL="anthropic/claude-sonnet-4-20250514"
export AI_CLI_API_KEY="$ANTHROPIC_API_KEY"

# AI_CLI_MODEL нужен только для opencode. Claude Code сам выбирает модель.
# Формат: provider/model (например: anthropic/claude-sonnet-4, openai/gpt-4o)

# OpenCode с кастомным API-эндпоинтом
export AI_CLI=opencode
...
|--------|-------------------|-------------------|-----------|
| **pro** | open-loop, problem-framing | Архитектура, сложный код, стратегия, multi-system изменения | Высокая |
| **thinking** | closed-loop | Типовые задачи, одно-файловые правки, написание контента | Средняя |
| **fast** | trivial | Переименование, обновление ссылок, форматирование, поиск файла, cron-агенты | Низкая |

Переключить модель в Claude Code: `/model` → выбрать. Для автоматических задач (Стратег, Экстрактор) рекомендуется fast — экономит ~80% лимита по сравнению с pro.

> **Как это работает — два сценария:**
> - **Вся сессия на другой модели:** При открытии Claude определяет класс верификации. Если задача trivial/closed-loop и текущая модель избыточна, Claude скажет: *«Рекомендую переключиться на [fast/thinking] через `/model`. Переключить автоматически я не могу.»* Пользователь переключает сам.
> - **Отдельная задача внутри сессии:** Если посреди сессии появилась trivial-задача, Claude делегирует её sub-agent'у на дешёвой модели. Основная сессия не прерывается. Делегирование только вниз (pro→thinking/fast, thinking→fast). Переключение вверх — только через `/model`.
>
> **Совет:** На подписке Claude Pro ($20/мес) активно используйте fast-тир для рутины (утренние планы, поиск по файлам, тривиальные правки). pro — только для архитектурных решений и сложного кода.
...
# Должны быть файлы памяти (platform + user)
ls ~/IWE/FMT-exocortex-template/persistent-memory/
ls ~/IWE/FMT-exocortex-template/workspaces/*/memory/
...
# Создай директорию и скопируй (fallback)
mkdir -p ~/IWE/workspaces/default-project/memory
cp ~/IWE/FMT-exocortex-template/seed/MEMORY.md ~/IWE/workspaces/default-project/memory/
...
Claude прочитает CLAUDE.md и persistent-memory/ и проведёт тебя через:
...
1. Открой `workspaces/<твой-workspace>/memory/day-rhythm-config.yaml`
...
|-----------|-------------|------------------------|
| CLAUDE.md, persistent-memory/ | Локальные файлы | Нет (только в контекст Claude при работе) |
...
Да. Стратег — это автоматизация (утренние планы, ревью). Без него Claude Code + CLAUDE.md + persistent-memory/ работают полностью. Планируешь вручную.
...
# Удалить файлы
rm ~/IWE/CLAUDE.md
rm -rf ~/IWE/workspaces/*/memory/persistent-memory && bash setup.sh
