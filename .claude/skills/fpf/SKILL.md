...
4. **Если iwe-knowledge недоступен** (нет в `/mcp`):
   ...
   - SPF: читай `spf/docs/` (относительно корня репо) через Glob + Read. Если `spf/` не существует → «SPF не клонирован локально, принципы SPF недоступны»
   - FPF: читай `fpf/Readme.md` (обзор) или ищи через Grep по `fpf/`. Если `fpf/` не существует → «FPF не клонирован локально, первые принципы недоступны»
