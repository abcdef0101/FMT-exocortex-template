...
> 1. `{{ROOT_DIR}}/roles/extractor/config/routing.md` — таблицы маршрутизации (Pack'и, типы, директории)
> 2. `{{ROOT_DIR}}/roles/extractor/config/feedback-log.md` — лог отклонённых кандидатов (не предлагай аналогичные)
...
### Шаг 3.5: Двойной routing — domain vs implementation
...
Каждое доменное понятие Pack'а обязано иметь родительское понятие из базовой онтологии SPF. Если доступен `{{WORKSPACE_DIR}}/SPF/ontology.md` — найди подходящее U.* понятие. Если SPF недоступен — оставь поле пустым, укажи `# TODO: привязать к SPF`.
...
> Для кандидатов, прошедших тест как «реализационное» в шаге 3.5.
...
> Если MCP knowledge-сервер недоступен — пропусти MCP-шаги, полагайся на чтение файлов.

Для каждого кандидата:

1. **MCP-поиск:** `knowledge_search("тема кандидата")` — есть ли похожее в базе?
   Если MCP недоступен → `grep -ri "ключевые термины кандидата" "$WORKSPACE_DIR"/PACK-*/pack/`
2. **MCP-документ:** `knowledge_get_document("целевой файл")` — проверить содержимое (если файл существует)
   Если MCP недоступен → `find "$WORKSPACE_DIR"/PACK-*/ -name "*целевой файл*" | head -1 | xargs Read`
...
4. Для defer — запиши в `{{WORKSPACE_DIR}}/DS-strategy/inbox/` для следующего цикла.

### Шаг 8a: Пометка captures

> После применения accept-кандидатов — пометить обработанные captures в `{{WORKSPACE_DIR}}/DS-strategy/inbox/captures.md`.
